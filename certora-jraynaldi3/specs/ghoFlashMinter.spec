...
rule availableLiquidityDoesntChange(method f, address token){
    ...
    uint256 _liquidity = maxFlashLoan(token);

    f(e, args);

    uint256 liquidity_ = maxFlashLoan(token);
...
// only Pool admin can modify several variabel 
rule onlyPoolAdmin(
    env e,
    method f,
    calldataarg args
) {
    uint256 feeBefore = getFee();
    address treasuryBefore = getGhoTreasury();

    f(e,args);

    uint256 feeAfter = getFee();
    address treasuryAfter = getGhoTreasury();
    assert (treasuryAfter != treasuryBefore )
        || (feeAfter != feeBefore) 
        => retreivePoolAdminFromGhost(e.msg.sender);
}

// fee cannot more than 10% 
invariant feeLimit()
    getFee() <= MAX_FEE();

// flashLoan only for ghoToken
rule ghoOnlyFlashLoan(
    env e, address receiver, address token, uint256 amount, bytes data
) {
    flashLoan@withrevert(e, receiver, token, amount, data);
    assert token != gho => lastReverted;
}

// flashLoan should call flash borrower
rule flashLoanOnSuccess(
    env e, address receiver, address token, uint256 amount, bytes data
) {
    uint8 _counter = flashBorrower.counter();
    flashLoan(e, receiver, token, amount, data);
    uint8 counter_ = flashBorrower.counter();
    assert _counter < counter_;
}

// flashLoan Should not changed totalSupply
rule totalSupply_monoton(
    env e ,
    method f,
    calldataarg args
) {
...
// maxFlashLoan should represent maximum amount(cannot loan more than max)
rule maxFlashLoan_CannotMore(
    env e, address receiver, address token, uint256 amount, bytes data
) {
    flashLoan@withrevert(e, receiver, token, amount, data);
    bool isRevert = lastReverted;
    assert amount > maxFlashLoan(gho) => isRevert;
}

// maxFlashLoan should represent maximum amount(can loan exactly same as max)
rule maxFlashLoan_CanExact(
    env e, address receiver, address token, uint256 amount, bytes data
) {
    require amount == maxFlashLoan(gho);
    flashLoan@withrevert(e, receiver, token, amount, data);
    satisfy !lastReverted;
}

// maxFlashLoan other than gho should 0
rule maxFlashLoanZero(address token){
    assert token != gho => maxFlashLoan(token) == 0;
}

// flashFee not support other token
rule flashFeeZero(env e, address token, uint256 amount) {
