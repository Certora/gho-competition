...
//addFacilitator should changed facilitator bucket capacity if the address is not in facilitator 
rule addFacilitator_integrity(
	env e,
	address facilitator,
	string label,
	...
	assumeInvariants(facilitator);
	bool isAlreadyIn = inFacilitatorsList(toBytes32(facilitator));

	addFacilitator(e, facilitator, label, bucketCapacity);

	assert to_mathint(GhoTokenHelper.getFacilitatorBucketCapacity(facilitator)) == to_mathint(bucketCapacity)
		<=> !isAlreadyIn
		&& hasRole(e, FACILITATOR_MANAGER_ROLE(e) , e.msg.sender) ;

	assert GhoTokenHelper.getFacilitatorsLableLen(facilitator) > 0;
}

// every non view rule should have atleast a way to execute without revert
rule sanity(
	env e,
	method f,
	calldataarg args
	...
	f@withrevert(e,args);
	if (f.selector == sig:mint(address,uint256).selector) {satisfy !lastReverted;}
	if (f.selector == sig:setFacilitatorBucketCapacity(address,uint128).selector) {satisfy !lastReverted;}
	if (f.selector == sig:removeFacilitator(address).selector) {satisfy !lastReverted;}
	if (f.selector == sig:burn(uint256).selector) {satisfy !lastReverted;}
	if (f.selector == sig:addFacilitator(address, string, uint128).selector) {satisfy !lastReverted;}
	else {satisfy !lastReverted;}
}

//TODO access control and overflow 
